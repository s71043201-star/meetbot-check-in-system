...
// 課程記錄暫存（30 分鐘自動清理）
const DOC_TTL = 30 * 60 * 1000;
const docStore = new Map();
function storeDoc(html, fileName) {
  const uid = Date.now().toString(36) + Math.random().toString(36).slice(2);
  docStore.set(uid, { html, fileName, createdAt: Date.now() });
  return uid;
}
setInterval(() => {
  const now = Date.now();
  for (const [uid, doc] of docStore) {
    if (now - (doc.createdAt || 0) > DOC_TTL) docStore.delete(uid);
  }
}, 5 * 60 * 1000);
...
// ── 跟課班表系統（Firebase 命名空間 /schedule/*，與簽到資料隔離）──
...
app.post("/checkout", async (req, res) => {
  ...
  try {
    ...
    res.json({ ok: true, hours, uid, downloadUrl });
  } catch (e) {
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  // ── Self-ping keep-alive（每 14 分鐘，防 Render 免費方案休眠）──
